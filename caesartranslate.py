...
class CaesarTranslate:
    ...
    def run():
        if len(sys.argv) <= 1:
            ...
            if sys.argv[1] == "help":
                print("CaesarAI Translate Help: runtranslate <filename> <subject> <small/large>")
            if sys.argv[1] == "ls_all":
                if "CaesarAudioWAVs" in os.listdir():
                    subjects= os.listdir("CaesarAudioWAVs")
                    for subject in subjects:
                        for audiofile in os.listdir(f"CaesarAudioWAVs/{subject}"):
                            print(f"{subject}: {audiofile}")
            if sys.argv[1] == "ls_todo":
                if len(sys.argv) == 3:
                    picked_subject = sys.argv[2].capitalize()
                else:
                    picked_subject =  None
                if "CaesarNotes" in os.listdir():
                    done_translations = []
                    todo = []
                    subject_todo = []
                    subjects= os.listdir("CaesarNotes")
                    for subject in subjects:
                        for audiofile in os.listdir(f"CaesarNotes/{subject}"):
                            done_translations.append(audiofile)
                    for subject in subjects:
                        for audiofile in os.listdir(f"CaesarAudioWAVs/{subject}"):
                            if picked_subject == subject:
                                todo.append(audiofile)
                                subject_todo.append(subject)
                            if picked_subject == None:
                                todo.append(audiofile)
                                subject_todo.append(subject)

                    print("CaesarNotes to Translate:")
                    for ind,audiotodo in enumerate(todo):
                        if audiotodo.replace(".wav",".txt") not in done_translations:
                            print(f"- {subject_todo[ind]}:{audiotodo}")
                            


        elif len(sys.argv) != 4:
            print(sys.argv)
            print("Not right amount of arguments. 3 arguments")
        elif len(sys.argv) == 4:
            ...
            filename = "CaesarAudioWAVs/{}/{}.wav".format(subject,argfilename)
            ...
            if largewav == "small":
                sentences =""
                txtfilename = "CaesarNotes/{}/{}.txt".format(subject,argfilename)
